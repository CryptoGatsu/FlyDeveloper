...
STATE_EXAMPLE = {
    ...
    "fly": {"name": "The Fly Dev", "symbol": "FLYDEV", "brain": "connectome", "neurons": 138639,
            ...
            "genesis": {"symbol": "FLYDEV", "name": "The Fly Dev", "token": "0x...", "curve": "0x...", "pons": "https://www.ponsfamily.com/launchpad/0x...",
                        ...
                        "market": {"at": "...", "source": "curve", "price_usd": 0.000012, "mcap_usd": 12345.0, "change_24h": 4.2, "change_basis": "24h",
                                   "price_quote": 0.00000005, "mcap_quote": 50.0, "quote_usd": 245.1, "graduation_pct": 12.0, "graduated": False,
                                   "series": [{"ts": 1.7e9, "mcap_usd": 12000.0}]}}},
...
- Every page carries the house coin strip: <div id="flycoin"></div> right after the header plus <script src="/coin.js"></script> at the end of the body (provided by the house; do not write it). It shows the $FLYDEV contract address (fly.genesis.token) with a copy button, the market cap and the 24 h move (fly.genesis.market). You may also chart fly.genesis.market.series (market cap over the last 24 h) on "/" and on /coins; never invent numbers, and show nothing when market is null. Do not duplicate the address in the header; you may repeat it on /coins.
...
- /browsing shows, IN THIS ORDER, newest first: FIRST the pages it read with their screenshots big and up top (state.pages: shot image, title, link, gist, need spotted, followups), THEN what it learned (state.learnings: summary + ideas), THEN the searches (state.searches: query, engine, result titles+urls). Visitors come to see what the fly is looking at right now, so the screenshots lead. Make it read like a fly's field notes, not a log dump. Keep the page short: render only the newest 6 pages, 4 learnings and 5 searches, each list followed by a "show more" button that reveals the next batch in place (no page reload, no "#" links); the state holds up to 60 pages and the page must not grow with it.
